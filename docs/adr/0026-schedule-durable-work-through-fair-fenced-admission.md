...
Each Admission Grant starts unbound and binds exactly once to one Scheduler Work Item, its immutable Task enactment OperationID and payload digest, one grant generation, and the durable C03 start acceptance. C03 start acceptance, Scheduler downstream `Accepted`, and Admission Grant `Bound` linearize in one Platform PostgreSQL transaction through a restricted Scheduler participant. C03 cannot create a second admission request or rebind the canonical start.
...
- Scheduler may release logical Workspace and policy counters after exact Runtime-fenced-or-terminal evidence under its own epoch and policy CAS. Physical node capacity remains quarantined until Runtime Execution separately proves process termination and containment/reset. `ReleaseReady` is therefore a physical-capacity gate, not a logical-counter gate.
