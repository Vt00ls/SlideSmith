...
- For a Runtime enactment, that transaction also creates the Scheduler Work Item through the restricted Scheduler transactional participant. The Work Item references the exact Task enactment OperationID and canonical C03 start-payload digest; delivery is not allowed to assemble or rewrite the start request later.
