...
- dequeue as accepted and grant binding: the C03 start-acceptance transaction,
  using a restricted Scheduler transactional participant, that atomically
  records durable C03 acceptance for the exact operation/digest, marks the Work
  Item Accepted, and moves that exact Admission Grant generation from
  ReservedUnbound to Bound;
...
An Admission Grant binds Work Item identity and generation, Task enactment
OperationID and payload digest, existing Runtime Run, selected Execution Node
and node-capacity generation, exact Resource Class and Execution Policy,
Scheduler epoch/policy, Reservation binding when applicable, grant identity and
monotonic generation, and expiry. The grant authenticates an already canonical
...
run another admission algorithm, or enqueue an admission operation after
accepting start. Task Orchestration, workers, brokers, C04, and Gateway likewise
cannot form another admission authority.
...
The exact evidence classes are intentionally separate. `RuntimeFencedOrTerminal`
...
the other module's facts directly.
...
The resulting grant is unbound. Authenticated delivery carries the immutable
Task enactment plus this grant to C03. C03 acceptance is the next step, not a
request for another admission decision. Sandbox Lease, C04/Gateway
prerequisites, Execution Capsule, and worker dispatch all occur after the
Accepted/Bound transaction.
...
- Re-admitting that Work Item preserves its Task enactment OperationID and
  payload digest and creates a new Admission Grant identity and higher
  generation. It is allowed only after Scheduler proves the earlier grant was
  unbound and expired/released and C03 inspection proves no accepted start.
- C03 exact replay with the same start/digest and same grant generation returns
  the original acceptance or rejection. A newer grant generation is a fresh
  admission proof only while no start has been accepted. If start is already
  accepted, C03 returns the original Decision and the extra grant is stale and
  releasable; it cannot rebind the start.
...
- Node loss may release logical counters after exact
  `RuntimeFencedOrTerminal` evidence and Scheduler CAS, while physical capacity
  stays unknown and quarantined until `PhysicalCapacityReleaseReady`.
...
- logical-counter release and lag separately from node readiness expiry,
  physical quarantine, containment, reset, release readiness, and capacity
  debt;
