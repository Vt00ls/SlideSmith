...
- Restore advances Scheduler and Runtime node/lease generations. Recovery controllers apply node-execution fences through C03's protected internal maintenance port, not by adding variants to public Runtime `Execute`. Exact `RuntimeFencedOrTerminal` evidence may let Scheduler release logical counters under its new epoch, but every restored or returning node remains physically quarantined until fresh readiness plus C03 termination and containment/reset evidence produces `PhysicalCapacityReleaseReady`. No live Admission Grant, Sandbox Lease, Runtime View, Gateway Grant, or worker operation is restored.
