...
- Restore advances Scheduler and Runtime node/lease generations. Recovery controllers apply node-execution fences through C03's protected operational `RuntimeMaintenance` interface, not by adding variants to public Runtime `Execute`. Exact `RuntimeFencedOrTerminal` evidence may let Scheduler release logical counters under its new epoch. An Accepted/Bound run with authoritative proof that no lease transaction committed terminates as pre-process `Rejected` and emits `NoLeasePhysicalDisposition`; it is not called `Lost`. Ambiguous lease state remains quarantined and reconciles the exact operation. A run with a possible lease/process may become `Lost`, and every restored or returning node remains physically quarantined until fresh readiness plus C03 termination and containment/reset evidence produces `PhysicalCapacityReleaseReady`. No live Admission Grant, Sandbox Lease, Runtime View, Gateway Grant, or worker operation is restored.
...
- [Workspace Export and Purge](./workspace-export-and-purge.md) treats purged backup bytes as inaccessible residue for at most the normal 35-day window, blocks purge while an exceptional pin would exceed it, uses an independent Purge Fence as irreversible suppression authority, and requires tombstone application before restore promotion;
