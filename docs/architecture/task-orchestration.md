...
defines the delivery boundary, and [task-workspace-lifecycle.md](./task-workspace-lifecycle.md)
defines C04 commit semantics.
...
- The Platform Control Plane remains authoritative for Task, Pipeline Version
  locks, Phase Run and Runtime Run state, and commit decisions.
- A Task pins its Pipeline Version after Route selection. Runtime Release and
  Template locks are immutable Task references and never float on retry,
  recovery, cancellation, or manual edit.
...
| --- | --- | --- |
| Task revision, coarse state, Route, pinned lock references, current Phase cursor, cancellation and retry decision | Task Orchestration in Platform PostgreSQL | Owns and changes through `Decide` |
| Immutable Phase graph, gates, contracts, retry eligibility, required capabilities, and manual-edit entry point | Pipeline publication / release authority | Reads only the exact pinned Pipeline Version |
...
- the selected Route and immutable Pipeline, Runtime Release, and Template
  lock references;
...
flowchart LR
    ...
    Pinned[Pinned Pipeline Version] --> Decide
...
Route selection is a pre-Pipeline Task decision. It atomically records the
Route and exact approved Pipeline Version before the first Phase Run is
created. It is not fabricated as a Phase Run that lacks a pinned Pipeline
Version. Runtime Release and Template locks are supplied by their owning
publication decisions and must be exact and durable before the first Phase
that consumes them; Task Orchestration never resolves a floating version.
...
Route. Issue 22 fixes the manifest identity and compatibility contract for
these immutable definitions without changing this seam.
...
1. An authorized start intent resolves a Route through platform policy and
   atomically records the exact approved Pipeline Version. Other required
   immutable locks must already be recorded before their first consuming
   Phase.
...
- Retry and recovery retain every Task lock. They cannot resolve `latest`,
...
- Task and accepted transition metadata, pinned locks, Phase Run history, and
...
- all three Route graphs, each Confirmation Gate, zero and multiple Runtime
  Runs per Phase Run, and fixed locks across the complete lifecycle;
...
never fabricate a Pipeline lock, Runtime Run relationship, successful Phase,
...
- Issue 22 supplies immutable Pipeline/Runtime publication identities,
  compatibility, activation, revocation, and pinning details. Its Pipeline
  Version must carry route Phase graphs, Confirmation Gates, contracts, retry
  eligibility, required capabilities, and the manual-edit entry point consumed
  here.
- Issue 24 receives idempotent Runtime enactments bound to Task, Phase Run,
  Runtime Run, capability, immutable inputs, and fences; it returns typed
  evidence and never chooses Phase outcome or the next Phase.
...
- Issue 17 maps legacy records to the Task revision, pinned locks, Phase Run
...
method and serialized state names, queue technology, retry timing, release
manifest fields, worker protocol, scheduling policy, telemetry backend, and
legacy batch sequence belong to the named downstream decisions or to adapter
SPECs and do not reopen this interface.
