...
4. **输入绑定**：每个 request 绑定 exact Task/Phase/Runtime identity、immutable input manifest、Pipeline/Runtime Release、guest digest、capability/resource class 和 Sandbox Lease fence。Agent Compose project revision/image ref 只能补充，不能替代这些 bindings。
   Proposed [ADR 0029](../adr/0029-bind-runtime-admission-once-before-post-lease-prerequisites.md)在explicit Owner approval并merge后要求：Task Orchestration 的 Runtime enactment 与 Scheduler Work Item 先原子提交；Scheduler `ClaimAndAdmit` 后，C03 原子接受 start/绑定 grant，随后才 grant Sandbox Lease、用 current `SandboxLeaseAuthority` open C04 Runtime View、取得 Gateway prerequisites、构造 Execution Capsule并调用 Agent Compose。Agent Compose adapter 不得创建 admission、Runtime View或第二个 start digest。
