...
本文记录 GitHub Issue #15 确认的 owner、Share Link 与 break-glass 内容授权契约。`CONTEXT.md` 是领域语言权威；ADR 0013、ADR 0014 与 ADR 0025 记录长期安全选择；`durable-object-storage.md` 定义授权后 verified content 的唯一读取 seam；`workspace-export-and-purge.md` 定义 one-shot export acceptance、Purge Fence 与 terminal Workspace access；`backup-and-recovery.md` 定义 restore 后的授权失效语义；`observability-audit-and-cleanup-debt.md` 定义 correlation、authoritative audit、telemetry、alert 与 retention 契约。
...
后一项只允许[Workspace Export and Purge](./workspace-export-and-purge.md) module创建generation-fenced machine operation。它不给requester或approver交互式浏览整个Workspace的能力，也不把export attempt、HTTP response start、partial delivery或missing receipt提升为purge authority。
...
- [Observability and audit contract](./observability-audit-and-cleanup-debt.md) 获得grant/session/generation、rate-limit、verification、content-open、break-glass、audit backlog、privacy和authoritative-vs-projection facts。
- [Workspace Export and Purge](./workspace-export-and-purge.md)使用dual-control BreakGlass Grant、one-shot `PrepareWorkspaceExport`、no interactive Workspace browsing和separate purge authorization，并在Purge Fence后terminally拒绝所有旧authority paths。
